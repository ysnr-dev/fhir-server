...
# drifting out of conformance with the vendored IG profiles (JP Core, JASPEHR).
# Fhir::Profile.mode defaults to :warn, so a non-conformant helper doesn't fail
# any *other* spec -- this is the one place that would notice, which matters
# before anyone flips FHIR_PROFILE_VALIDATION to :enforce in production.
RSpec.describe "IG profile payload helper conformance", type: :request do
  def create_patient
  ...
  Fhir::ResourceRegistry::ENTRIES.each_key do |resource_type|
    ...
    next unless Fhir::Profile::DefinitionStore.known_profile?(entry[:profile])
    ...
    it "keeps valid_#{resource_type.underscore}_payload conformant with #{entry[:profile]}" do
      ...
      expect(result.valid?).to be(true), "#{resource_type} payload helper has profile issues:\n#{diagnostics}"
    end
  end
end
