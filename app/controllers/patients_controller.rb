class PatientsController < ApplicationController
  RESOURCE_TYPE = "Patient".freeze

  before_action :set_patient, only: %i[history vread]
  ...
  def render_not_found
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "not-found",
      diagnostics: "Patient/#{params[:id]} not found"
    )
  end
  ...
  def render_gone
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "deleted",
      diagnostics: "Patient/#{params[:id]} has been deleted"
    )
  end
