class PractitionersController < ApplicationController
  RESOURCE_TYPE = "Practitioner".freeze

  before_action :set_practitioner, only: %i[history vread]
  ...
  def history
    return render_not_found unless @practitioner

    versions = Fhir::Repository.history(RESOURCE_TYPE, @practitioner.id)
    bundle = BundleBuilder.history(resource_id: @practitioner.id, versions: versions, base_url: base_url, resource_type: RESOURCE_TYPE)
  ...
  def vread
    ...
    version = Fhir::Repository.version(RESOURCE_TYPE, @practitioner.id, params[:vid].to_i)
  ...
  def render_not_found
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "not-found",
      diagnostics: "Practitioner/#{params[:id]} not found"
    )
  end
  ...
  def render_gone
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "deleted",
      diagnostics: "Practitioner/#{params[:id]} has been deleted"
    )
  end
