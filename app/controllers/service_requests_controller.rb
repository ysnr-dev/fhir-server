class ServiceRequestsController < ApplicationController
  RESOURCE_TYPE = "ServiceRequest".freeze

  before_action :set_service_request, only: %i[history vread]
  ...
  def history
    return render_not_found unless @service_request

    versions = Fhir::Repository.history(RESOURCE_TYPE, @service_request.id)
    bundle = BundleBuilder.history(resource_id: @service_request.id, versions: versions, base_url: base_url, resource_type: RESOURCE_TYPE)
  ...
  def vread
    ...
    version = Fhir::Repository.version(RESOURCE_TYPE, @service_request.id, params[:vid].to_i)
  ...
  def render_not_found
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "not-found",
      diagnostics: "ServiceRequest/#{params[:id]} not found"
    )
  end
  ...
  def render_gone
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "deleted",
      diagnostics: "ServiceRequest/#{params[:id]} has been deleted"
    )
  end
