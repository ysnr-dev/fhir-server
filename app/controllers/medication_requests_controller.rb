class MedicationRequestsController < ApplicationController
  RESOURCE_TYPE = "MedicationRequest".freeze

  before_action :set_medication_request, only: %i[history vread]
  ...
  def set_medication_request
    @medication_request = MedicationRequest.find_by(id: params[:id])
  end
  ...
  def render_not_found
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "not-found",
      diagnostics: "MedicationRequest/#{params[:id]} not found"
    )
  end
  ...
  def render_gone
    render_operation_outcome_single(
      ...
      severity: "error",
      code: "deleted",
      diagnostics: "MedicationRequest/#{params[:id]} has been deleted"
    )
  end
