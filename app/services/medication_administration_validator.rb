class MedicationAdministrationValidator < ResourceValidator
  private

  def validate
    require_field("status") && validate_binding("status", Fhir::Terminology::MEDICATION_ADMINISTRATION_STATUS)
    validate_medication
    validate_subject
  end
  ...
  def validate_medication
    ...
    add_error(
      ...
      expression: %w[MedicationAdministration.medicationCodeableConcept MedicationAdministration.medicationReference]
    )
  end
