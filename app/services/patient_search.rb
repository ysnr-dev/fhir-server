class PatientSearch
  ...
  def call
    scope = Patient.where(deleted: false)
  ...
  def filter_identifier(scope)
    ...
    ids = if value.include?("|")
            ...
            PatientIdentifier.where(system: system, value: val).select(:patient_id)
          else
            PatientIdentifier.where(value: value).select(:patient_id)
          end
    scope.where(id: ids)
  end
