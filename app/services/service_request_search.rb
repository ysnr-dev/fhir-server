class ServiceRequestSearch
  ...
  def call
    scope = ServiceRequest.where(deleted: false)
  ...
  def filter_identifier(scope)
    ...
    ids = if value.include?("|")
            ...
            ServiceRequestIdentifier.where(system: system, value: val).select(:service_request_id)
          else
            ServiceRequestIdentifier.where(value: value).select(:service_request_id)
          end
    scope.where(id: ids)
  end
  ...
  def filter_code(scope)
    ...
    value = params["code"]
    scope.where(code: value).or(scope.where("code_text ILIKE ?", like_pattern(value)))
  end
