class PractitionerSearch
  ...
  def call
    scope = Practitioner.where(deleted: false)
    scope = filter_id(scope)
    scope = filter_identifier(scope)
    scope = filter_name(scope)
    scope = filter_family(scope)
    scope = filter_given(scope)
    scope = filter_gender(scope)
    scope = filter_birthdate(scope)
  ...
  def filter_identifier(scope)
    ...
    ids = if value.include?("|")
            ...
            PractitionerIdentifier.where(system: system, value: val).select(:practitioner_id)
          else
            PractitionerIdentifier.where(value: value).select(:practitioner_id)
          end
    scope.where(id: ids)
  end
  ...
  def filter_name(scope)
    ...
    scope.where("name_text ILIKE ?", like_pattern(params["name"]))
  end

  def filter_family(scope)
    return scope if params["family"].blank?

    scope.where("family ILIKE ?", like_pattern(params["family"]))
  end

  def filter_given(scope)
    return scope if params["given"].blank?

    scope.where("given ILIKE ?", like_pattern(params["given"]))
  end

  def filter_gender(scope)
    return scope if params["gender"].blank?

    scope.where(gender: params["gender"])
  end
  ...
  def filter_birthdate(scope)
    return scope if params["birthdate"].blank?

    prefix, date_str = split_prefix(params["birthdate"])
    date = parse_date(date_str)
    return scope unless date

    apply_comparison(scope, "birth_date", prefix, date)
  end
