...
flowchart TB
    ...
    subgraph serverRepo["fhir-server — Rails 8 / JP-Core v1.2.0 + JASPEHR v1.0.0"]
        REST["FHIR REST<br/>34 リソース / 検索 / _history / Bundle / $validate"]
...
| 領域 | 内容 |
|---|---|
| FHIR REST | 34 リソース（`Patient` / `Observation` / `MedicationRequest` / `Questionnaire` ほか）の CRUD、チェーン検索・`_has`・`_include`、`_history` / vread、条件付き操作、JSON Patch、`$validate`、`Patient/$everything`、Bundle（transaction / batch） |
...
- ルーティングは 34 リソースへ同一のルートセットを生成し、`FhirResourcesController` に集約
